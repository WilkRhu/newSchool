        ...
        if(!error){
            if(value.type === "student") {
                ...
                const data = {
                    ...
                    student_resposible_one: student.student_resposible_one,
                    student_resposible_two: student.student_resposible_two,
                    date_of_birth: student.date_of_birth
                };
                const studant = await crateStudantes(createUser[0].id, data);
                if (studant.error) { 
                    await connect("users").where("id", createUser[0].id).del();
                    return res.status(400).json(studant.error.message);
                }
